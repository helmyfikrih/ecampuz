...
function addNew() {
	$("#form").trigger("reset");
	$(".modalHeaderText").html("Add New Instansi");
	showModal();
...
function view(e) {
	...
	jQuery.ajax({
		...
		url: `${base_url}instansi/getOne`,
		dataType: "json",
		success: function (e) {
			$(".modalHeaderText").html(`Detail Instansi ${e[0].instansi_name}`);
			var d = new Date();
			var t = d.getTime();
			$("#body-view").html(
				`<div class="row">
					...
					<table class="table">
						<tbody>
							<tr>
								...
								<td>${e[0].instansi_name}</td>
							</tr>
							<tr>
								<th>Deskripsi :</th>
								<td>${e[0].instansi_description}</td>
							</tr>
						</tbody>
					</table>
				</div>
...
function edit(e) {
	...
	jQuery.ajax({
		...
		dataType: "json",
		success: function (e) {
			jQuery("#instansi_id").val(e[0].instansi_id);
			jQuery("#nama").val(e[0].instansi_name);
			jQuery("#deskripsi").val(e[0].instansi_description).trigger("change");
		},
		error: function (xhr, status, error) {
			Swal.fire({ title: "Error", text: error, icon: "error" });
		},
